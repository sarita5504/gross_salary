...
class Add_warehouse:
    def __init__(self, root):
        ...
        self.root.title("Manage Warehouse")
        self.root.config(bg="white")
        
        self.var_status=StringVar()
        self.var_name=StringVar()  

        var1=Label(self.root,text="Add Warehouse",bg='white',fg='grey',font='comicsansms 16 bold',borderwidth=1,relief=RIDGE, pady=5)
        var1.pack(fill=X)

        lbl_name=Label(self.root,text="Warehouse Name :",font=("goudy old style",15),bg="white").place(x=10,y=40)
        self.txt_name=Entry(self.root, textvariable=self.var_name, font=("goudy old style",13),bg="white")
        self.txt_name.place(x=185,y=40)
    ...
    def database_connector(self):
        if self.var_name.get()=="":
            ...
            my_cursor.execute("INSERT INTO warehouse (name, status) VALUES (%s, %s)", (self.var_name.get(), self.var_status.get()))
            conn.commit()
            conn.close()
